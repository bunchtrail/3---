...
def genetic_algorithm():
    ...
    # Автоматическая генерация родительских маршрутов
    num_parents = 4
    parents = initialize_parents(num_parents, cities)

    # Генерация потомков с использованием изощрённого кроссинговера
    offspring = []
    crossover_info = []  # Сохранение информации о точках кроссинговера
    ...
    for idx in range(num_parents):
        parent = parents[idx]
        child = offspring[idx]
        cp1, cp2 = crossover_info[idx // 2]  # Получение точек кроссинговера для данной пары

        # Рассчитываем стоимость маршрута родителя
        parent_cost, parent_detail = calculate_route_cost(parent, distance_matrix, city_indices)
        # Рассчитываем стоимость маршрута потомка
        child_cost, child_detail = calculate_route_cost(child, distance_matrix, city_indices)
        
        # Формирование генотипа родителя с разделителями звездочками
        parent_genotype = ''.join(parent[:cp1]) + '*' + ''.join(parent[cp1:cp2+1]) + '*' + ''.join(parent[cp2+1:])
        child_genotype = ' -> '.join(child)
    ...
    print(f"Маршрут: {' -> '.join(best_offspring)}")
